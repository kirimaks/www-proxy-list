...
Run create_proxy_list.py".format(DB_PATH))
    ...
    def get_https_proxy(self):
        query = "SELECT * FROM proxy_list WHERE protocol = 'https'\
    ...
    def make_query(self, query):
        try:
            ...
            self.logger.error("Something wrong with database, \
run create_proxy.py, exit...")
            sys.exit(2)

        out = self.cursor.fetchone()
        pattern = u"{protocol}://{addr}:{port}"

        proxy = pattern.format(protocol=out[3].strip(), addr=out[0].strip(),
                               port=out[1].strip())
        return proxy
